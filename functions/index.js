...
exports.addAdminRole = onCall(async (request) => {
  ...
  try {
    ...
    return {
      message: `성공! ${email} 사용자에게 admin 역할이 부여되었습니다.`,
    };
  } catch (error) {
...
// ✅ 3. 교사 계정 요청 처리 함수
exports.requestTeacherAccount = onCall(
  { region: "asia-northeast3", cors: true },
  async (request) => {
    const { name, email, password, requestedClassId, classDocId } =
      request.data;
...
exports.approveTeacher = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      const studentDocRef = db.doc(
        `classes/${generatedClassId}/students/${teacherUid}`
      );
      ...
      return {
        success: true,
        classId: generatedClassId,
        classDocId: requestedClassDocId,
      };
    } catch (error) {
...
exports.createStudentAccounts = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      if (!callerDoc.exists)
        throw new Error("호출자 사용자 문서가 존재하지 않습니다.");
    ...
    const finalClassId =
      callerRole === "teacher" ? callerClassId : classIdToAssign;
    if (!finalClassId) {
  ...
  async (request) => {
    ...
    for (const uid of studentUids) {
      try {
        ...
        await Promise.all([userRef.delete(), classStudentRef.delete()]);
...
exports.resetPasswordHttp = onRequest(
  ...
  async (req, res) => {
    if (req.method !== "POST") {
      console.warn("Method Not Allowed for resetPasswordHttp:", req.method, {
        structuredData: true,
      });
    ...
    if (!authorizationHeader.startsWith("Bearer ")) {
      console.warn(
        "Unauthorized for resetPasswordHttp: Missing or malformed Bearer token.",
        {
          structuredData: true,
        }
      );
      return res
        .status(401)
        .send({ error: "Unauthorized", message: "인증 토큰이 필요합니다." });
    }
    ...
    try {
      ...
      console.error("Error verifying ID token for resetPasswordHttp:", error, {
        structuredData: true,
      });
      return res
        .status(401)
    ...
    console.info(
      `Request received from UID: ${callerUid} for resetPasswordHttp`,
      { structuredData: true }
    );
    ...
    if (!callerDoc.exists) {
      ...
      console.error(
        `Caller document not found for UID: ${callerUid} in resetPasswordHttp.`,
        {
    ...
    if (callerRole !== "admin" && callerRole !== "teacher") {
      ...
      return res
        .status(403)
        .send({ error: "Forbidden", message: "권한이 없습니다." });
    }

    if (callerRole === "teacher" && !callerClassId) {
      const errorMsg =
        "교사에게 학급 정보가 없습니다. 비밀번호를 초기화할 수 없습니다.";
      console.error(
        `Teacher ${callerUid} has no classId for resetPasswordHttp.`,
        {
    ...
    if (!studentUidToReset) {
      console.warn("Bad Request for resetPasswordHttp: Student UID missing.", {
        structuredData: true,
      });
      return res
        .status(400)
        .send({ error: "Bad Request", message: "학생 UID가 필요합니다." });
    }
    ...
    if (!studentDocSnap.exists) {
      ...
      return res
        .status(404)
        .send({ error: "Not Found", message: "학생 정보를 찾을 수 없습니다." });
    }

    const studentData = studentDocSnap.data();
    if (callerRole === "teacher") {
      const studentClassId =
        studentData.classId || studentData.className || null;
      if (studentClassId !== callerClassId) {
        const warnMsg =
          `Forbidden attempt by teacher ${callerUid} (Class: ${callerClassId}) ` +
    ...
    try {
      ...
      const errorLogContext = `Error resetting password for student ${studentData.email} (UID: ${studentUidToReset}):`;
...
exports.fmpProxy = onRequest(
  ...
  async (req, res) => {
    ...
    if (!apiKey)
      return res
        .status(500)
        .send("Server configuration error: API key missing.");
    if (!endpointType)
      return res.status(400).send("Missing endpoint type parameter.");

    if (endpointType === "quote") {
      if (!symbolsParam && !symbolParam) {
        return res
          .status(400)
          .send("Missing symbols or symbol parameter for quote endpoint.");
      }
        ...
        symbols.map(async (symbol) => {
          try {
            ...
            if (snap.exists) {
              ...
              const age =
                (Timestamp.now().seconds - data.lastFetched.seconds) / 3600;
      ...
      if (!symbolParam || !timeframe) {
        return res
          .status(400)
          .send(
            "Missing symbol or timeframe parameter for historical-chart endpoint."
          );
      }
      ...
      try {
        ...
        if (snap.exists) {
          ...
          const age =
            (Timestamp.now().seconds - data.lastFetched.seconds) / 3600;
          if (
            age < CHART_CACHE_DURATION_HOURS &&
            Array.isArray(data.historical) &&
            data.historical.length > 0
          ) {
            const processed = processFmpHistoricalData(
              data.historical,
              timeframe
            );
  ...
  async (_event) => {
    ...
    try {
      ...
      const nyDate = new Date(
        nowKST.toLocaleString("en-US", { timeZone: "America/New_York" })
      );
      ...
      if (
        nyDayOfWeek === 0 ||
        nyDayOfWeek === 6 ||
        US_STOCK_MARKET_HOLIDAYS.includes(nyDateStr)
      ) {
      ...
      await Promise.all([
        db
          .collection("prefetchControl")
          .doc("status_quotes")
          .set(quotesStatus, { merge: true }),
        db
          .collection("prefetchControl")
          .doc("status_charts")
          .set(chartsStatus, { merge: true }),
      ]);

      console.log(
        `[Reset Prefetch Status Daily] 완료 - 기준일자(NY): ${nyDateStr}, RunType: ${runType}`
      );
    } catch (error) {
...
exports.prefetchQuotesData = onSchedule(
  ...
  async () => {
    ...
    while (idx < symbols.length && processed < 25) {
      ...
      while (retries <= 1 && !success) {
        try {
          ...
          if (quote) {
            batch.set(
              db.collection("stockMarketCache").doc(symbol),
              {
                currentPriceUSD: quote.price,
                previousCloseUSD: quote.previousClose,
                name: quote.name,
    ...
    if (processed) {
      await batch.commit();
      await db
        .collection("stockMarketSummary")
        .doc(`quotes_${targetNY}`)
        .set(summary, { merge: true });
    }
    await statusRef.set(
      {
        lastProcessedSymbolIndex: idx - 1,
        dailyRunCompleted: idx >= symbols.length,
        targetDate: targetNY,
      },
      { merge: true }
    );
...
exports.prefetchChartsData = onSchedule(
  ...
  async () => {
    ...
    while (idx < symbols.length && processed < 25) {
      ...
      while (retries <= 1 && !success) {
        try {
          const from = dayjs(targetNY)
            .subtract(1, "year")
            .add(1, "day")
            .format("YYYY-MM-DD");
          const res = await axios.get(
            `https://financialmodelingprep.com/api/v3/historical-price-full/${symbol}?from=${from}&to=${targetNY}&apikey=${apiKey}`
          );
          const hist = res.data.historical;
          if (Array.isArray(hist))
            batch.set(
              db.collection("stockChartCache").doc(`${symbol}_D_fmp`),
              { historical: hist, lastFetched: Timestamp.now(), source: "fmp" },
              { merge: true }
            );
          success = true;
        } catch (e) {
...
exports.rebuildStockSummaries = onSchedule(
  ...
  async () => {
    ...
    try {
      ...
      await db
        .collection("stockMarketSummary")
        .doc("today_quotes_all")
        .set(quoteSummary);
      console.info(
        `[rebuild] Quotes summary saved. Total symbols: ${
          Object.keys(quoteSummary).length
        }`
      );
      ...
      chartSnap.forEach((d) => {
        ...
        if (dt.historical) {
          ...
          console.info(
            `[rebuild] Chart generated for ${sym}. Keys:`,
      ...
      console.info(
        `[rebuild] Total symbols with chart data: ${
      ...
      if (!mapData || Object.keys(mapData).length === 0) {
        console.warn(
          "[rebuild] mapData is empty or undefined, skipping chart summary save"
        );
        return null;
      }
      ...
      console.info(
        `[rebuild] Calling splitAndSetDoc with collectionName=${collectionName}, docIdPrefix=${docIdPrefix}`
      );
...
exports.manualPrefetchStockProfiles = onRequest(
  // 함수 이름 변경 및 onRequest 사용
  {
    region: "asia-northeast3",
  ...
  async (req, res) => {
    ...
    console.info(
      "========== [Manual Prefetch Stock Profiles] Function execution STARTED by HTTP request =========="
    );
    const apiKey = fmpApiKeyParam.value();
    if (!apiKey) {
      console.error(
        "[Manual Prefetch Stock Profiles] CRITICAL: FMP API key not available."
      );
      return res
        .status(500)
        .send({ error: "Server configuration error: API key missing." });
    }
    ...
    try {
      ...
      if (allSymbols.size === 0) {
        console.info(
          "[Manual Prefetch Stock Profiles] No symbols found in stockCategories."
        );
        return res
          .status(200)
          .send({
            message:
              "No symbols found in stockCategories. Nothing to prefetch.",
          });
      }

      const symbolsArray = Array.from(allSymbols);
      console.info(
        `[Manual Prefetch Stock Profiles] Found ${symbolsArray.length} unique symbols to prefetch profiles for.`
      );
        ...
        try {
          ...
          if (
            ...
            if (profileData && profileData.symbol) {
              ...
              successCount++;
              console.info(
                `[Manual Prefetch Stock Profiles] Prepared cache for ${symbol}.`
              );
            } else {
              console.warn(
                `[Manual Prefetch Stock Profiles] Invalid profile data structure for ${symbol}:`,
                profileDataArray
              );
              errorCount++;
            }
            ...
            console.warn(
              `[Manual Prefetch Stock Profiles] No profile data (empty array) returned for ${symbol}.`
            );
            errorCount++; // 혹은 'not found'로 별도 카운트
          } else {
            console.warn(
              `[Manual Prefetch Stock Profiles] Failed to fetch profile for ${symbol}. Status: ${response.status}, Data:`,
              response.data
            );
            errorCount++;
          }
          ...
          console.error(
            `[Manual Prefetch Stock Profiles] Error fetching FMP profile for ${symbol}:`,
            error.message
          );
          errorCount++;
          if (error.response && error.response.status === 429) {
            // Rate limit
            console.warn(
              `[Manual Prefetch Stock Profiles] Rate limit possibly hit for ${symbol}. Waiting before next call.`
            );
      ...
      if (successCount > 0) {
        // 성공적으로 가져온 데이터가 있을 때만 배치 커밋
        await firestoreBatch.commit();
        console.info(
          `[Manual Prefetch Stock Profiles] Firestore batch committed. ${successCount} profiles cached/updated.`
        );
      }
      ...
      return res
        .status(200)
        .send({ message: summaryMessage, successCount, errorCount });
    } catch (error) {
      console.error(
        "[Manual Prefetch Stock Profiles] CRITICAL OVERALL ERROR:",
        error
      );
      return res
        .status(500)
        .send({
          error: "Overall error during profile prefetch: " + error.message,
        });
    } finally {
      console.info(
        "========== [Manual Prefetch Stock Profiles] Function execution FINISHED =========="
      );
    }
  }
);
...
async function fetchDailyPricesAdmin() {
  ...
  try {
    ...
    if (summarySnap.exists) {
      ...
      console.warn(
        "⚠️ [fetchDailyPricesAdmin] stockMarketSummary/today_quotes 문서가 없습니다."
      );
    }
  } catch (error) {
    console.error(
      "❌ [fetchDailyPricesAdmin] 일일 시세 정보 가져오는 중 오류:",
      error
    );
  }
...
async function fetchClassConversionRateAdmin(classId) {
  try {
    if (!classId) {
      console.warn(
        "[fetchClassConversionRateAdmin] classId가 제공되지 않았습니다. 기본 환율 1을 사용합니다."
      );
      return 1;
    }
    ...
    console.warn(
      `⚠️ [fetchClassConversionRateAdmin] 학급 설정 문서(ID: ${classId})가 없거나 conversionRate 필드가 없습니다.`
    );
  } catch (error) {
    console.error(
      `❌ [fetchClassConversionRateAdmin] 학급 환율 정보 가져오는 중 오류 (ID: ${classId}):`,
      error
    );
  }
 ...
 */
async function updateStudentStockValueInFirestore(
  classId,
  userId,
  currentPricesMap,
  conversionRate
  ...
  const portfolioRef = db
    ...
    .collection("students")
    .doc(userId)
    .collection("stockPortfolio");

  // 디버깅: 생성된 경로가 올바른지 확인
  console.log(
    `[updateStudentStockValueInFirestore] studentRef path: ${studentRef.path}`
  );
  console.log(
    `[updateStudentStockValueInFirestore] portfolioRef path: ${portfolioRef.path}`
  );
  ...
  try {
    ...
    console.log(
      `[updateStudentStockValueInFirestore] Portfolio size: ${portfolioSnap.size}`
    );

    if (portfolioSnap.empty) {
      console.warn(
        `[updateStudentStockValueInFirestore] Student's portfolio is empty.`
      );
    } else {
      console.log(
        `[updateStudentStockValueInFirestore] Looping through portfolio documents...`
      );
    }
    ...
    portfolioSnap.docs.forEach((docSnap) => {
      ...
      console.log(
        `[updateStudentStockValueInFirestore] Doc ID: ${docSnap.id}, Data:`,
        stockData
      );
      ...
      if (symbol && currentPricesMap[symbol] !== undefined) {
        ...
        if (!isNaN(quantity) && quantity > 0 && priceUSD > 0) {
          ...
          console.log(
            `[updateStudentStockValueInFirestore] Added to total: ${quantity} * ${priceUSD}. Current totalValueUSD: ${totalValueUSD}`
          );
        }
      } else {
        console.warn(
          `[updateStudentStockValueInFirestore] No price found for symbol: ${symbol} in pricesMap.`
        );
      }
    });

    console.log(
      `[updateStudentStockValueInFirestore] Final totalValueUSD before rounding: ${totalValueUSD}`
    );
    ...
    console.log(
      `[updateStudentStockValueInFirestore] Preparing to update Firestore with:`,
      stockValueData
    );
    ...
    console.error(
      `❌ Failed to update stockValue for student ${userId}:`,
      error
    );
    return false;
  }
...
exports.onTransactionCreateUpdatePortfolioValue = onDocumentCreated(
  ...
  async (event) => {
    ...
    if (!correctClassId || !correctUserId) {
      console.error(
        `Missing classId or userId field inside tradeHistory document [${tradeId}]`
      );
      return;
    }

    console.log(
      `Trade Record [${tradeId}] created for student ${correctUserId} in class ${correctClassId}. Type: ${tradeData.type}`
    );

    if (
      tradeData.type.startsWith("STOCK_BUY") ||
      tradeData.type.startsWith("STOCK_SELL")
    ) {
      console.log(
        `Transaction type [${tradeData.type}] requires portfolio value update. Proceeding...`
      );

      // ⭐ 올바른 ID를 사용하여 헬퍼 함수 호출
      const dailyPricesMap = await fetchDailyPricesAdmin();
      const conversionRate = await fetchClassConversionRateAdmin(
        correctClassId
      );
      await updateStudentStockValueInFirestore(
        correctClassId,
        correctUserId,
        dailyPricesMap,
        conversionRate
      ...
      console.log(
        `Transaction type [${tradeData.type}] does not require portfolio value update.`
      );
    }
    return;
  }
...
async function calculateSalaryTax(classId, studentUids, sinceTimestamp) {
  ...
  for (const uid of studentUids) {
    let q = query(
      collection(db, `classes/${classId}/students/${uid}/paySlips`)
    );
    if (sinceTimestamp) {
...
async function calculateClassStockExchangeFee(classId, sinceTimestamp) {
  let totalFeeDelta = 0;
  let q = query(
    collectionGroup(db, "tradeHistory"),
    where("classId", "==", classId),
    where("feeInClassCurrency", ">", 0)
  );
  if (sinceTimestamp) {
  ...
  async (request) => {
    ...
    try {
      ...
      const summaryDocRef = doc(
        db,
        `classes/${classId}/dashboardSummary/taxSummary`
      );
      const summarySnap = await getDoc(summaryDocRef);

      const lastUpdatedAt = summarySnap.exists
        ? summarySnap.data().incomeLastUpdatedAt
        : null;
      const currentSummary = summarySnap.exists ? summarySnap.data() : {};

      const studentUids = await getStudentUids(classId);
      const [salaryTax_delta, manualTaxPaid_delta, stockExchangeFee_delta] =
        await Promise.all([
      ...
      const newManualTaxPaid =
        (currentSummary.manualTaxPaid || 0) + manualTaxPaid_delta;
      const newStockExchangeFee =
        (currentSummary.stockExchangeFee || 0) + stockExchangeFee_delta;
      const newTotalIncome =
        newSalaryTax + newManualTaxPaid + newStockExchangeFee;
      ...
      await setDoc(
        ...
        { merge: true }
      ); // merge: true로 다른 필드(expense 등)는 덮어쓰지 않음

      console.log(`✅ [${classId}] 세금 수입 현황 업데이트 완료!`);
      return {
        success: true,
        message: "세금 수입 현황이 성공적으로 업데이트되었습니다.",
      };
    } catch (error) {
      console.error(`❌ [${classId}] 세금 수입 업데이트 실패:`, error);
      throw new HttpsError(
        "internal",
        "세금 수입 현황 업데이트 중 서버 오류가 발생했습니다."
      );
    }
  }
);
  ...
  async (request) => {
    ...
    try {
      ...
      await setDoc(
        ...
        { merge: true }
      ); // merge: true로 다른 필드(income 등)는 덮어쓰지 않음

      console.log(`✅ [${classId}] 세금 지출 현황 업데이트 완료!`);
      return {
        success: true,
        message: "세금 지출 현황이 성공적으로 업데이트되었습니다.",
      };
    } catch (error) {
      console.error(`❌ [${classId}] 세금 지출 업데이트 실패:`, error);
      throw new HttpsError(
        "internal",
        "세금 지출 현황 업데이트 중 서버 오류가 발생했습니다."
      );
    }
  }
);
...
exports.onDonationCreateUpdateSummary = onDocumentCreated(
  ...
  async (event) => {
    ...
    const summaryDocRef = db.doc(
      `classes/${classId}/dashboardSummary/donationSummary`
    );
...
exports.castVote = onCall(
  ...
  async (request) => {
    ...
    const voterRef = db.doc(
      `classes/${classId}/assemblyBills/${billId}/voters/${uid}`
    );
...
exports.closeVoting = onCall(
  ...
  async (request) => {
    ...
    return db.runTransaction(async (transaction) => {
      ...
      const billRef = db.doc(`classes/${classId}/assemblyBills/${billId}`);
      const criteriaRef = db.doc(
        `classes/${classId}/assemblySettings/criteria`
      );

      const [classDoc, billDoc, criteriaDoc] = await transaction.getAll(
        classRef,
        billRef,
        criteriaRef
      );
      ...
      if (
        criteriaDoc.exists &&
        typeof criteriaDoc.data().passThreshold === "number"
      ) {
        passThreshold = criteriaDoc.data().passThreshold;
      } else if (
        classDoc.exists &&
        typeof classDoc.data().studentCount === "number"
      ) {
...
exports.approveMission = onCall(async (request) => {
  ...
  if (rewards.credit && rewards.credit > 0) {
    const logRef = db.collection(
      `classes/${classId}/students/${studentUid}/creditLogs`
    );
    await logRef.add({
  ...
  return {
    success: true,
    message: "과제 제출이 승인되고 보상이 지급되었습니다.",
  };
});
 ...
 */
exports.processAutoApproveMission = onCall(
  { region: "asia-northeast3" },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError(
    ...
    try {
      const result = await db.runTransaction(async (transaction) => {
        ...
        const [missionDoc, studentDoc] = await transaction.getAll(
          missionRef,
          studentRef
        );

        if (!missionDoc.exists)
          throw new HttpsError("not-found", "존재하지 않는 도전과제입니다.");
        ...
        if (!mission.autoApprove)
          throw new HttpsError(
            "permission-denied",
            "이 과제는 자동 승인 대상이 아닙니다."
          );
        if (mission.completions && mission.completions[studentUid])
          throw new HttpsError("already-exists", "이미 제출한 도전과제입니다.");
        if (
          mission.targetStudentUids &&
          !mission.targetStudentUids.includes(studentUid)
        )
          throw new HttpsError(
            "permission-denied",
            "이 과제의 대상이 아닙니다."
        ...
        if (
          mission.repeatSchedule &&
          Object.keys(mission.repeatSchedule).length > 0
        ) {
          ...
          if (!(nowMin >= startH * 60 + startM && nowMin <= endH * 60 + endM))
            throw new HttpsError(
              "failed-precondition",
              "지금은 제출 가능한 시간이 아닙니다."
            );
        ...
        return {
          success: true,
          message: "도전과제가 자동 승인되고 보상이 지급되었습니다.",
        };
      });
      return result;
    } catch (error) {
